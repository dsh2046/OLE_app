    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            if UserProfile.objects.filter(email=user_name):
                return render(request, 'register.html', {'register_form': register_form, 'msg': 'User is already exist!'})
            else:
                ...
                # Write Thanks for registry
                user_message = UserMessage()
                user_message.user = user_profile.id
                user_message.message = 'Thanks for registry'
                user_message.save()
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if login_form.is_valid():
            ...
            pass_word = request.POST.get('password', '')
            user = authenticate(username=user_name, password=pass_word)   #Database verify
            if user is not None:
                if user.is_active:
                    login(request, user)
...
class ModifyPwdView(View):
    def post(self,request):
        ...
        if modify_form.is_valid():
            ...
            email = request.POST.get('email', '')
            if pwd1 != pwd2:
                return render(request, 'password_reset.html', {'email': email, 'msg': 'Password should be the same!'})
            else:
                user = UserProfile.objects.get(email=email)
                user.password = make_password(pwd2)
                user.save()
                EmailVerifyRecord.objects.get(email=email).delete()
                return render(request, 'login.html')
        else:
            email = request.POST.get('email', '')
...
class UpdatePwdView(View):
    def post(self, request):
        ...
        if modify_form.is_valid():
            ...
            pwd2 = request.POST.get('password2', '')
            if pwd1 != pwd2:
                return HttpResponse('{"status": "fail", "msg": "Password should be the same!"}')
            else:
                user = request.user
                user.password = make_password(pwd2)
                user.save()
                return HttpResponse('{"status": "success", "msg": "Password should be the same!"}')
        else:
...
class SendEmailCodeView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        if UserProfile.objects.filter(email=email):
            return HttpResponse('{"email": "Email already exist!"}')
        else:
...
class UpdateEmailView(LoginRequiredMixin, View):
    def post(self, request):
        ...
        if existed_records:
            ...
            return HttpResponse('{"email": "Verified code error!"}')
...
class MyMessageView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        # After clicking unread message icon, clean unread mark
